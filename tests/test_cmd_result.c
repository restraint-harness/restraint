...
test_rstrnt_port()
{
    ...
    g_assert_cmpstr(CURRENT_SERVER_STRING, ==, app_data->s.server);
...
test_environment_variables()
{
    ...
    g_assert_true(rc);
    g_assert_cmpstr(app_data->outputfile, ==, outputfile);
    g_assert_cmpstr(app_data->s.task_id, ==, task_id);
    g_assert_cmpstr(app_data->s.server_recipe, ==, recipe_url);
    g_assert_cmpstr(app_data->s.server, ==, server_url);
